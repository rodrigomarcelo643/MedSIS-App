...
import { View, Text, FlatList, Alert, Platform, Keyboard } from "react-native";
...
export default function ChatScreen() {
  ...
  const { id, name, avatar, user_type, isOnline, highlightMessage } =
  ...
  const hasThreeButtonNav = React.useMemo(
    () => (Platform.OS === "ios" ? insets.bottom > 20 : insets.bottom > 0),
    [insets.bottom],
  );
  ...
  useEffect(() => {
    ...
    const loadFromCache = async () => {
      if (user?.id && actualUserId) {
        try {
          const cached = await messageStorage.getMessages(
            user.id,
            actualUserId,
          );
    ...
    if (highlightMessage) {
      ...
      setTimeout(
        () => isMountedRef.current && setHighlightedMessageId(null),
        3000,
      );
    }
  ...
  const checkUserOnlineStatus = async () => {
    try {
      ...
      const { users } = await messageService.getActiveUsers(
        user?.id || "",
        1,
        100,
      );
  ...
  const loadMessages = async (pageNum = 1, append = false) => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/messages/get_messages.php?sender_id=${encodeURIComponent(user.id)}&receiver_id=${encodeURIComponent(actualUserId)}&page=${pageNum}&limit=20`,
      );
  ...
  const silentLoadMessages = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/messages/get_messages.php?sender_id=${encodeURIComponent(user.id)}&receiver_id=${encodeURIComponent(actualUserId)}&page=1&limit=20`,
      );
  ...
  const editMessage = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/messages/edit_message.php`,
        {
  ...
  const unsendMessage = async (messageId: string) => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/messages/unsend_message.php`,
      ...
      Alert.alert("Error", "Failed to unsend");
    } finally {
  ...
  const getInitials = (n: string) =>
    n
      ? n
          ...
          .slice(0, 2)
      : "U";
  const canEditMessage = (ts: string) =>
    (new Date().getTime() - new Date(ts).getTime()) / (1000 * 60) <= 3;
    ...
    <View style={{ flex: 1, backgroundColor }}>
      <ChatHeader
        ...
        onInfo={() => {
          ...
          router.push(
            `/chat-info/${safeId}?name=${name}&avatar=${chatAvatar || ""}&user_type=${user_type}&isOnline=${userOnlineStatus}`,
          );
      ...
      <ChatMessageList
        ...
        hasMore={hasMore}
        onLoadMore={() =>
          hasMore && !loadingMore && loadMessages(page + 1, true)
        }
        ...
        onImagePress={(url) => {
          const urls = messages
            ...
            .map((m) => m.fileUrl!);
          setImageCarousel(urls);
          setCurrentImageIndex(urls.indexOf(url));
